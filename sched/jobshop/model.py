      ...
      """
        ...
        if not self.is_parallel:
            ...
            for _, job in self.jobs.items():
                for t in job.tasks:
                    model.Add(
                        sum(task_durations[job.idx, t.group]) == t.duration)
        ...
        for _, job in self.jobs.items():
            ...
            for _prev, _next in zip(job.tasks[:_size - 1], job.tasks[1:]):
                model.Add(task_start[job.idx, _next.group] >=
                          task_end[job.idx, _prev.group])

        # makespan
        model.AddMaxEquality(makespan, [
            task_end[job.idx, job.tasks[-1].group]
            for _, job in self.jobs.items()
        ])
    ...
    def cp_to_gantt_mermaid(self, fp='result', start_date=None):
      ...
      """
        ...
        try:
            ...
            render_gantt_html(template_path=template_path,

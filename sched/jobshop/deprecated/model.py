...
class RawInstance:
    """
    object free instances;
    put here for reference.
    """
    logger = logging.getLogger(f"{__package__}")

    @staticmethod
    def jsp_generate_random_instance(m: int, n: int, copy: int = 1, density: float = 0.8, style='non-f'):
        """
        :param style:
            - non-f, regular jsp
            - f, fjsp, i.e., flexible jsp, for which each machine has copies
        :param m: num of jobs
    ...
    @staticmethod
    def fjsp(jobs_data, mcopy, pre=False, max_sol=20, max_sec=10):
        ...
        solution_printer = SatCallBack(obj_var, max_sol, max_sec)
...
if __name__ == '__main__':
    m, n, p = 50, 10, 3
    jobs_data, mcopy = RawInstance.jsp_generate_random_instance(m, n, copy=p, style='f', density=0.5)
    RawInstance.fjsp(jobs_data, mcopy, pre=False)
